...
class AnagrampuzzleValidationException implements Exception {
  final List<String> errors;

  AnagrampuzzleValidationException(this.errors);

  @override
  String toString() => 'AnagrampuzzleValidationException: ${errors.join("; ")}';
}
...
/// Anagram Puzzle Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/anagrampuzzle?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [word] - Specific word to create anagram for
/// * [count] - Number of puzzles [min: 1, max: 20]
/// * [difficulty] - Difficulty level
class AnagrampuzzleClient {
  final String apiKey;
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AnagrampuzzleValidationException(errors);
    }
  }

  /// Execute a request to the Anagram Puzzle Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<AnagrampuzzleResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AnagrampuzzleException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AnagrampuzzleResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AnagrampuzzleException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AnagrampuzzleException('Resource not found');
      } else {
        throw AnagrampuzzleException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AnagrampuzzleException) rethrow;
      throw AnagrampuzzleException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Anagram Puzzle Generator API client.
class AnagrampuzzleException implements Exception {
  final String message;

  AnagrampuzzleException(this.message);

  @override
  String toString() => 'AnagrampuzzleException: $message';
}
